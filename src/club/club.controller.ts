import {
  ...
  export class ClubController {
    ...
    @Get(':clubId')
    async findOne(@Param('clubId') clubdId: string) {
      return await this.clubService.findOne(clubdId);
    }
  
    @Post()
    async create(@Body() clubDto: ClubDto) {
      const club: ClubEntity = plainToInstance(
        ClubEntity,
        clubDto,
      );
    ...
    async update(
      ...
      return await this.clubService.update(
        clubId,
        club,
      );
    }
