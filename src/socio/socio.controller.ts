import {
  ...
  export class SocioController {
    ...
    @Get(':socioId')
    async findOne(@Param('socioId') sociodId: string) {
      return await this.socioService.findOne(sociodId);
    }
  
    @Post()
    async create(@Body() socioDto: SocioDto) {
      const socio: SocioEntity = plainToInstance(
        SocioEntity,
        socioDto,
      );
    ...
    async update(
      ...
      return await this.socioService.update(
        socioId,
        socio,
      );
    }
